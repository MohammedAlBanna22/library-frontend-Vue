  ...
  <div class="page">
    ...
    <div class="page-header">
      <div>
        ...
        <span class="page-sub">{{ total }} book{{ total !== 1 ? 's' : '' }} published</span>
      </div>
    ...
    <div class="toolbar">
      <div class="search-wrap">
        ...
        <input v-model="search" class="search-input" placeholder="Search by title, genre..." />
    ...
    <div v-else-if="!filteredBooks.length" class="empty-state">
      ...
      <p>No books found</p>
      <button class="btn-primary" @click="openCreate">Publish your first book</button>
    </div>
      ...
      <table>
        ...
        <tbody>
          <tr v-for="book in sortedBooks" :key="book.id" class="table-row">
            ...
            <td>
              <span class="date-text">{{ formatYear(book.published_at) }}</span>
            </td>
            <td>
              <div class="actions">
                <button class="action-btn edit" @click="openEdit(book)" title="Edit">
      ...
      <div v-for="book in sortedBooks" :key="book.id" class="book-card">
        <div class="card-cover">
          ...
          <div class="card-actions-overlay">
            <button class="action-btn edit" @click.stop="openEdit(book)" title="Edit">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"/></svg>
            </button>
            <button class="action-btn delete" @click.stop="confirmDelete(book)" title="Delete">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14H6L5 6"/><path d="M10 11v6M14 11v6"/><path d="M9 6V4h6v2"/></svg>
            </button>
          </div>
        </div>
        <div class="card-body">
          ...
          <div class="card-footer">
            <div>
              ...
              <span class="copies-info">
                <span class="available-copies">{{ book.available_copies }}</span>/<span class="total-copies">{{ book.total_copies }}</span>
              </span>
            </div>
      ...
      <div v-if="showModal" class="modal-backdrop" @click.self="closeModal">
        <div class="modal modal-lg">
          <div class="modal-header">
            <h2>{{ editingId ? 'Edit Book' : 'Publish New Book' }}</h2>
          ...
          <form @submit.prevent="submitForm" class="modal-body">
            <div class="form-grid">
              ...
              <div class="field span-2">
                ...
                <textarea v-model="form.description" rows="4" placeholder="Tell readers about your book..."></textarea>
            ...
            <div class="modal-footer">
              ...
              <button type="submit" class="btn-primary" :disabled="submitting">
                {{ submitting ? 'Saving...' : (editingId ? 'Save Changes' : 'Publish Book') }}
              </button>
            </div>
          </form>
        </div>
...
const availableCount = computed(() => books.value.filter(b => b.is_available).length)
const totalCopies = computed(() => books.value.reduce((sum, b) => sum + (b.total_copies || 0), 0))
...
const fetchBooks = async () => {
  ...
  try {
    ...
    const data = res.data
    books.value = data.data ?? (Array.isArray(data) ? data : [])
  } catch (e) {
...
const openCreate = () => {
  ...
  form.value      = { title: '', isbn: '', description: '', genre: '', published_at: '', total_copies: 1, price: '', cover_image: '' }
  errors.value    = {}
  formError.value = ''
...
.header-stats {
  display: flex; gap: 16px; align-items: center; margin-top: 18px; flex-wrap: wrap;
}
.stat-card {
...
.stat-label { font-size: 11px; color: #999; text-transform: uppercase; letter-spacing: 0.05em; font-weight: 500; }
.stat-value { font-size: 18px; font-weight: 600; color: #333; }
.stat-value.available { color: #16a34a; }
...
.copies-label { font-size: 11px; color: #999; margin-right: 4px; }
...
.card-footer { display: flex; align-items: center; justify-content: space-between; margin-top: 6px; font-size: 12px; }
.card-price { font-weight: 600; color: #c9a96e; }
